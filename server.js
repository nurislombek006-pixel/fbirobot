...
async function webSend(req,res){
  try{
    const id=await resolveDialogId(req);
    if(!id)return json(res,{ok:false,error:'Чат не найден.'},404);
    const d=await dialogMeta(id);
    if(!d)return json(res,{ok:false,error:'Чат не найден.'},404);
    const text=String(req.body?.text||'').trim();
    if(!text)return json(res,{ok:false,error:'Напиши сообщение перед отправкой.'},400);
    if(text.length>4096)return json(res,{ok:false,error:'Сообщение слишком длинное. Максимум 4096 символов.'},400);
    if(!d.business_connection_id)return json(res,{ok:false,error:'Этот чат не подключён через Telegram Business, поэтому отправить сообщение от имени профиля нельзя.'},400);

    const cr=await q('select is_enabled,rights from business_connections where id=$1',[d.business_connection_id]);
    const conn=cr.rows[0];
    if(conn && conn.is_enabled===false)return json(res,{ok:false,error:'Подключение Telegram Business сейчас отключено.'},403);
    if(conn?.rights && conn.rights.can_reply!==true){
      return json(res,{ok:false,error:'У бота нет права отвечать от имени профиля. В Telegram Business → Чат-боты включи разрешение на ответы.'},403);
    }
    ...
    if(!result?.ok){
      const raw=String(result?.description||result?.raw||result?.error||'Ошибка Telegram');
      let friendly=raw;
      if(/24 hour|24-hour|last 24|reply.*24/i.test(raw)) friendly='Telegram разрешает бизнес-боту отвечать только в личных чатах, где было входящее сообщение за последние 24 часа.';
      else if(/not enough rights|rights|can_reply|forbidden/i.test(raw)) friendly='У бота недостаточно прав для ответа. Проверь разрешение на ответы в Telegram Business → Чат-боты.';
      else if(/business connection/i.test(raw)) friendly='Telegram Business-подключение недоступно или было отключено. Переподключи бота в настройках Telegram Business.';
      return json(res,{ok:false,error:friendly},400);
    }

    const m=result.result||{};
    const ts=Number(m.date||unix());
    const peer=d.peer||{};
    const own=d.owner||{};
    const stored={
      dialog_id:d.id,
      ...
      side:'right',
      from_id:String(own.id||''),
      author:own.short||own.name||'Вы',
      author_full:own.name||own.short||'Вы',
      to_id:String(peer.id||d.chat_id||''),
      to_name:peer.name||peer.short||'',
      text,
      plain:text,
      date_ts:ts,
      time_text:fmt(ts),
    ...
    return json(res,{ok:true,message:rowMsg(stored),telegram_message_id:stored.message_id});
  }catch(e){
    console.error('webSend error:',e);
    return json(res,{ok:false,error:'Не удалось отправить сообщение: '+String(e?.message||e)},500);
  }
}
...
async function sendWebMessage(){
  if(sending)return;
  const input=document.getElementById('compose'),btn=document.getElementById('sendBtn'),text=input.value.trim();
  if(!text)return;
  ...
  try{
    ...
    if(!d.ok)throw new Error(d.error||'Не удалось отправить');
    input.value='';input.style.height='auto';
    if(d.message){const before=msgMap.size;mergeMessages([d.message]);if(msgMap.size>before)total++;render();requestAnimationFrame(()=>scrollTo({top:document.body.scrollHeight,behavior:'smooth'}));}
    toast('Сообщение отправлено');
    setTimeout(()=>poll(true),500);
  }catch(e){
    toast(String(e.message||e));
  }finally{
    sending=false;btn.disabled=false;btn.textContent='➤';input.focus();
  }
}
function resizeCompose(){const x=document.getElementById('compose');x.style.height='auto';x.style.height=Math.min(x.scrollHeight,130)+'px'}
function refreshNow(){poll(true)}function clearSearch(){const x=document.getElementById('search');if(!x.value)return;x.value='';searchQ='';render();x.focus()}let st;document.getElementById('search').addEventListener('input',e=>{clearTimeout(st);st=setTimeout(()=>{searchQ=e.target.value.trim();render()},100)});document.getElementById('compose').addEventListener('input',resizeCompose);document.getElementById('compose').addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendWebMessage()}});window.addEventListener('scroll',updateJump,{passive:true});initial();setInterval(()=>poll(false),4000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)poll(true)});
</script></body></html>`}
...
app.post('/api/send',(req,res)=> okKey(req)?webSend(req,res):json(res,{ok:false,error:'Forbidden'},403));
